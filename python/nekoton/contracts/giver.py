...
class IGiver:
    async def give(self, target: _nt.Address, amount: _nt.Tokens):
        raise NotImplemented("IGiver is an abstract class")


class GiverV1(IGiver):
    @staticmethod
    def compute_address(workchain: int = 0) -> _nt.Address:
        return _nt.Address.from_parts(workchain, _nt.Cell.decode(_giver_v1_tvc).repr_hash)

    @staticmethod
    async def deploy(transport: _nt.Transport, workchain: int = 0, other_giver: IGiver | None = None) -> GiverV1:
        ...
        elif state.status == _nt.AccountStatus.Uninit and state.balance < initial_balance:
            tx = await other_giver.give(address, initial_balance)
            if tx is None:
                raise RuntimeError("Message expired")
            await transport.trace_transaction(tx).wait()
